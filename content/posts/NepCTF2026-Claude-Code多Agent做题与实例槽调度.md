...
| **zip 下载（推荐 v0.2.1）** | [ctf-agent-dispatch-0.2.1.zip](https://github.com/fjh1997/ctf-agent-dispatch/releases/download/v0.2.1/ctf-agent-dispatch-0.2.1.zip) |
| Release 页 | [v0.2.1](https://github.com/fjh1997/ctf-agent-dispatch/releases/tag/v0.2.1)（含根 `CLAUDE.md` + `.claude/agents/`） |

```bash
curl -L -o ctf-agent-dispatch-0.2.1.zip \
  https://github.com/fjh1997/ctf-agent-dispatch/releases/download/v0.2.1/ctf-agent-dispatch-0.2.1.zip
...
兼容：**Claude Code**（`claude`）/ **Codex**（读根目录 `AGENTS.md`）/ 纯 CLI。  
强制安全见仓库 **`SAFETY.md`**：禁止格式化真盘、禁止 Ghost 还原到盘符/PhysicalDrive。

> 说明：不是「不需要 agents 配置」。v0.1.0 只放了 `agents/CLAUDE.md` 命名偏内部；**v0.2.1 起以根 `CLAUDE.md` + `.claude/agents/` 为准**。
